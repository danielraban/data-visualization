...
Bubbles = () ->
  ...
  rScale = d3.scale.sqrt().range([40,maxRadius])
  ...
  # (which is what Daniel's original version does)
  # ---
  collide = (jitter) ->
  ...
  updateActive = (id) ->
    ...
    if id.length > 0
      d3.select("#status").html("<h3>The expense <span class=\"active\">#{id}</span> is now active</h3>")
    else
      d3.select("#status").html("<h3>No expense is active</h3>")
...
texts = [
  {key:"testdata",file:"top_data.csv",name:"Test Data"}
]
